...
import { mysqlTableNames, QueryResult, TableNames, GetById, GetAll, PasswordFields } from "../../types/type.js"
...
//? Custom queries
const customQuery = async (tableName: TableNames, sql: string, values: Array<string | number | boolean>): Promise<QueryResult> => {
    try {
...
const dBFetch = async (tableName: TableNames, sql: string, values?: Array<string | number | boolean>): Promise<QueryResult> => {
    ...
    if (sql.trim().split(";").length > 1) return { success: false, error: "Multiple SQL statements are not allowed." }
    // Check if the SQL query string and values are not only present but also valid.
    if (!sql || typeof sql !== "string") return { success: false, error: "Invalid input: SQL query or values are missing or incorrect." }
    ...
    if (Object.keys(result).length === 0) return { success: true, error: "Nothing has been found with the query" }
