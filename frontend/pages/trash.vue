...
const recover = (id: string) => {
    for (let i = 0; i < passwordStore.trashPasswords.length; i++) {
        if (passwordStore.trashPasswords[i].id === id) {
            ...
            if (passwordStore.trashPasswords[i].favorite) passwordStore.favoritePasswords.push(passwordStore.trashPasswords[i])
        }
    }
...
const removePermanently = (id: string) => {
    confirm.require({
        ...
        accept: () => {
            toast.add({ severity: "success", summary: "Success", detail: "The password block has been removed", life: 3000 })
            remove(id)
        },
        reject: () => {
            toast.add({ severity: "error", summary: "Cancelled", detail: "Deleting password block has been cancelled", life: 3000 })
        },
    })
}
</script>
                ...
                <PHolder password-type="login" v-for="block in passwordStore.trashPasswords">
                    <template #title>
                        ...
                        <div class="space-x-1">
                            ...
                            <Button severity="danger" text outlined @click="removePermanently(block.id)">Delete Permanently</Button>
                        </div>
                    </template>
                    <template #date>
